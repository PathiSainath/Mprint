...
const BookmarksPage = () => {
  ...
  const fetchBookmarks = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/products/category/bookmarks?${params}`, {
        method: "GET",
        headers: {
                  ...
                  return (
                    <div key={product.id} className="group">
                      <Link to={`/bookmarks/${product.slug}`}>
                        <div className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 relative overflow-hidden h-full flex flex-col">
                          <button
