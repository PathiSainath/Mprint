...
import { FaRegHeart, FaHeart, FaStar, FaPhotoVideo, FaFilter } from "react-icons/fa";
...
const FramesPage = () => {
  ...
  const fetchFramesProducts = async () => {
    setLoading(true);
    setError("");
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/products/category/frames?${params}`,
        {
          method: "GET",
      ...
      setError("Failed to load photo frames from backend.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getProductImageUrl = (product) => {
    ...
    return "https://via.placeholder.com/400x500?text=Photo+Frame";
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-16 px-6">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-gray-600">Loading photo frames...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="px-6 md:px-10 py-10 bg-gradient-to-b from-pink-50 to-gray-50 min-h-screen">
      {/* Header Section */}
      <div className="mb-10">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">Photo Frames</h1>
          <p className="text-lg text-gray-600 mb-2">
            Beautiful custom photo frames to showcase your cherished memories
          </p>
          <p className="text-sm text-pink-600">
            Premium materials • Custom sizes • Fast delivery • Perfect for gifts
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto mb-10">
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <h3 className="text-2xl font-bold text-pink-600">{products.length}+</h3>
              <p className="text-sm text-gray-600">Frame Designs</p>
            </div>
            <div>
              <h3 className="text-2xl font-bold text-purple-600">Custom</h3>
              <p className="text-sm text-gray-600">Sizes & Styles</p>
            </div>
            <div>
      ...
      {isMobile && (
        <div className="max-w-7xl mx-auto mb-6">
          ...
          >
            <div className="absolute inset-0 bg-gradient-to-r from-pink-600 via-pink-500 to-purple-600 rounded-xl"></div>
            <div className="absolute inset-0 bg-gradient-to-r from-pink-400 to-purple-400 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl"></div>
          ...
          <div className="flex-1">
            {products.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-lg shadow-md">
                ...
                <h3 className="text-2xl font-semibold text-gray-800 mb-4">No Photo Frames Available</h3>
                ...
                {products.map((product) => {
                  ...
                  return (
                    <div key={product.id} className="group">
                      <Link to={`/frames/${product.slug}`}>
                        <div className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 relative overflow-hidden border border-pink-100">
                          <button
                          ...
                          <div className="aspect-square overflow-hidden">
                            <img
                              ...
                              onError={(e) => {
                                e.target.src = "https://via.placeholder.com/400x400?text=Photo+Frame";
                              }}
                            />
                          </div>
                          <div className="p-5">
                            <h2 className="text-lg font-semibold text-gray-800 mb-2 line-clamp-2 group-hover:text-pink-600 transition-colors">
                              {product.name}
                            </h2>
