...
import { FaRegHeart, FaHeart, FaStar, FaRegSmile, FaFilter } from "react-icons/fa";
...
const GreetingCardsPage = () => {
  ...
  const fetchGreetingCards = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/products/category/greeting-cards?${params}`, {
        method: "GET",
        headers: {
      ...
      setError("Failed to load greeting cards from backend.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getProductImageUrl = (product) => {
    ...
    return "https://via.placeholder.com/400x300?text=Greeting+Card";
  };
  ...
  return (
    <div className="px-6 md:px-10 py-10 bg-gradient-to-b from-green-50 to-gray-50 min-h-screen">
      {/* Header */}
      <div className="mb-10">
        <div className="max-w-7xl mx-auto text-center">
          <h1 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">💌 Greeting Cards</h1>
          <p className="text-lg text-gray-600 mb-2">
            Beautiful greeting cards for every occasion and mood
          </p>
          <p className="text-sm text-green-600">
            Custom designs • Premium cardstock • Fast printing & delivery
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto mb-10">
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <h3 className="text-2xl font-bold text-green-600">{products.length}+</h3>
            ...
            <div>
              <h3 className="text-2xl font-bold text-purple-600">Delightful</h3>
      ...
      {isMobile && (
        <div className="max-w-7xl mx-auto mb-6">
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-green-400 to-emerald-400 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl"></div>
          ...
          <div className="flex-1">
            {products.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-lg shadow-md">
                ...
                <h3 className="text-2xl font-semibold text-gray-800 mb-4">No Greeting Cards Found</h3>
          ...
          {products.map((product) => {
            ...
            return (
              <div key={product.id} className="group">
                <Link to={`/greetingcards/${product.slug}`}>
                  <div className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 relative overflow-hidden border border-green-100">
                    <button
                    ...
                    <div className="aspect-[4/3] overflow-hidden bg-gray-50">
                      <img
                        ...
                        onError={(e) => {
                          e.target.src = "https://via.placeholder.com/400x300?text=Greeting+Cards";
                        }}
                      />
                    </div>
                    <div className="p-5">
                      <h2 className="text-lg font-semibold text-gray-800 mb-2 line-clamp-2 group-hover:text-green-600 transition-colors">
                        {product.name}
                      </h2>
